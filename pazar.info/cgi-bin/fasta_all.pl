...
# Author: Jonathan Lim
# Notes: This file is just a copy of tf_search.cgi with all the HTML output disabled or stripped out. Output is replaced with that similar to fasta_call.pl
# except that this file will be used to display all regseqs that a transcription factor interacts with.
# input parameters: results (expected value: all)
# Output: plain text file for download. Content is stored in the variable $fasta and printed to file
#
...
print "Content-Disposition:attachment;filename=PAZAR_$pazartfid"."_seqs.txt\n\n";
print $fasta;
